...
class CocoEvaluator(object):
    def __init__(self, coco_gt: COCO, iou_types: list):
    ...
    def prepare(self, predictions: dict, iou_type: str):
        if iou_type == "bbox":
    ...
    def prepare_for_coco_detection(self, predictions: dict):
        coco_results = []
    ...
    def prepare_for_coco_segmentation(self, predictions: dict):
        coco_results = []
        for original_id, prediction in predictions.items():
            if len(prediction) == 0:
                continue
...
def evaluate(coco_eval: COCOeval):
    """
    ...
    if p.useSegm is not None:
        ...
        logger.info(f"useSegm is set. Running {p.iouType} evaluation.")
    ...
    computeIoU = coco_eval.computeIoU if p.iouType in ['segm', 'bbox'] else coco_eval.computeOks
    coco_eval.ious = {(imgId, catId): computeIoU(imgId, catId)
                      for imgId in p.imgIds for catId in catIds}

    evalImgs = [
        coco_eval.evaluateImg(imgId, catId, areaRng, p.maxDets[-1])
